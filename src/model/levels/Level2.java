...
public class Level2 extends Level implements Serializable{
	...
	private static final long serialVersionUID = 1437975597688032201L;
	...
	public void createWaves() {
		...
		for (int i = 0; i < numbOfWaves; i++){
			...
			for (int j = 0; j < 5; j++){
				int enemyGenerator = r.nextInt(20);  // chooses a random between 0 and 9 
				if (enemyGenerator <= 2){
	...
	@Override
	public void setMap() {
		Map levelsMap = MapFactory.generateMap(getPlayer(), 2);
		 //the int mapCode is 2 because this is level 2 and we want Map 2 (Cave Map)
